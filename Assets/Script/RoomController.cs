...
public class RoomController : MonoBehaviour
{
    ...
    public EnnemyController[] ennemyController; //potentiellemnt une variable a ajout� pour les items et mettre une liste d'ennemy
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("Player"))
        {
            for (int i = 0; i < ennemyController.Length; i++)
            {
                ...
                {
                    Destroy(collision.gameObject); // peut afficher � la place la lose sc�ne
                }
            }
